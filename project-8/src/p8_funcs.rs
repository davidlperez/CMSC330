...
pub fn load_string_upper(fname: &str) -> Vec<String> {
  ...
  reader.lines().map(|l| l.expect("Could not parse line").to_uppercase()).collect()
}
...
pub fn mark_corrected(text: &String, dict: &Vec<String>) -> String {
  let dictionary: HashSet<String> = dict.iter().cloned().collect();
  ...
  for word_match in word_regex.find_iter(text) {
    ...
    corrected.push_str(&text[last_end..word_match.start()]);
    last_end = word_match.end();

    if dictionary.contains(&word.to_uppercase()) {
...
where T: Corrector                               // 3rd param must impl Corrector to have correct_word() function
{
  ...
  for word_match in word_regex.find_iter(text) {
    ...
    if dictionary.contains(&word.to_uppercase()) {
      ...
      corrected.push_str(&corrector.correct_word(word));
    }
  }
...
impl AutoCorrector {
  ...
  pub fn closest_word(&self, word: &str) -> (String,usize) {
    ...
    self.dict_words.iter().map(|w| (w.clone(), edit_distance(&word_upcase, w))).min_by(|a, b| a.1.cmp(&b.1)).unwrap_or((String::new(), usize::MAX))
  }
}
...
impl Corrector for AutoCorrector {
  ...
  fn correct_word(&mut self, word: &str) -> String{
    ...
    if self.show_sub {
      format!("({}:{}:{})", word, closest_word, distance)
    } else {
      closest_word
    }
  }
