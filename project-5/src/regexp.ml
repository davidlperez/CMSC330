...
let regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  let rec convert = function
    ...
    | Char c -> char_nfa c
    | Union (r1, r2) -> let nfa1 = convert r1 in
      let nfa2 = convert r2 in
      let start = fresh () in
      let delta_union = (start, None, nfa1.q0) :: (start, None, nfa2.q0) :: nfa1.delta @ nfa2.delta in
      {
        sigma = union nfa1.sigma nfa2.sigma;
        qs = start :: (nfa1.qs @ nfa2.qs);
        q0 = start;
        fs = nfa1.fs @ nfa2.fs;
        delta = delta_union;
      }
    | Concat (r1, r2) -> let nfa1 = convert r1 in
      let nfa2 = convert r2 in
  let delta_concat = (List.map (fun f -> (f, None, nfa2.q0)) nfa1.fs) @ nfa1.delta @ nfa2.delta in
      {
        sigma = union nfa1.sigma nfa2.sigma;
        qs = nfa1.qs @ nfa2.qs;
        q0 = nfa1.q0;
    ...
    | Star r -> let nfa = convert r in
      let start = fresh () in
      let delta_star = (start, None, nfa.q0) :: (List.map (fun f -> (f, None, nfa.q0)) nfa.fs) @ nfa.delta in
      {
        sigma = nfa.sigma;
        qs = start :: nfa.qs;
        q0 = start;
